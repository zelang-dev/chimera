 ...
 */
static void gifDestroy(void *pointer) {
 ...
 * is the total length.
 */
static int gifAddData(void *pointer, byte *data, int len, bool data_ended) {
	gifState *gs = (gifState *)pointer;
	int rval;
	int pos;
...
static Image *gifGetImage(void *pointer) {
	gifState *gs = (gifState *)pointer;
	return gs->image;
}
 ...
 */
void gifInit(FormatLineProc lineProc, void *closure, struct ifs_vector *if_vector) {
	gifState *gs;

	gs = (gifState *)malloc(sizeof(gifState));
