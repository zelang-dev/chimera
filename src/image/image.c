...
static byte *lf_get_8_bit_map(MemPool mp, Intensity *old_table) {
...
static bool lf_null_gray_map(byte *maps[3]) {
	int i;
	for (i = 0; i < 256; i++) {
...
static Image *lf_get_image(ImageState *is) {
...
static void lf_set_up_bitmap(ImageState *is) {
...
static void lf_expand_bitmap_to_true(
...
static void lf_expand_bitmap_to_gray(
	byte *expansion_buffer,
	byte *source_buffer,
	int width,
	Intensity *red_cmap,
...
static void lf_set_up_expansion(is)
ImageState *is;
{
...
static void lf_set_up_dither(is)
ImageState *is;
{
	byte *maps[3];
	int i;
...
static void lf_get_new_specials(is, ic, image, input)
ImageState *is;
ImageClass *ic;
Image *image;
byte *input;
...
static void ImageToXImage(void *pointer, int fline, int lline) {
	...
	if (is->xi == NULL) {
	   ...
	   */
		...
		if (image_type == IRGB || image_type == ITRUE) {
			if (!ic->colorcube) {
				ic->colorcube =	xccf_allocate_cube(is->dpy, is->cmap, is->v, is->depth);
			}
		}

		if ((image_type == IGRAY || !ic->colorcube) && image->pixlen != 1) {
			if (!ic->grayscale) {
				ic->grayscale = xccf_allocate_grays(is->dpy, is->cmap, is->v, is->depth);
			}
		}
  ...
  /* Make sure we have plenty of padding at the end of each line */
		...
		if (depth != 1)
			XAddPixel(is->xi, is->bgcolor.pixel);

		if (image->depth != 1 || is->expansion_buf)
			lf_set_up_dither(is);
  ...
  */
	for (line = fline;
		...
		if (image->pixlen == 1 && !is->expansion_buf) {
			memcpy(output, input, MIN(is->xi->bytes_per_line, image->bytes_per_line));
		} else {
			byte *expanded_input;
			if (is->expansion_buf) {
...
void *closure;
{
	...
	for (i = 0; i < is->free_dither_table; i++) {
		if (is->dither_table[i].generic_dither_table)
			free(is->dither_table[i].generic_dither_table);
 ...
 */
void *ImageInit(ChimeraRender wn, void *class_closure, void *state) {
 ...
 * ImageAdd
 */
static void ImageAdd(void *closure) {
	ImageState *is = (ImageState *)closure;
	byte *data;
	size_t len;
	MIMEHeader mh;
 ...
 * ImageEnd
 */
static void ImageEnd(void *closure) {
	ImageState *is = (ImageState *)closure;
	byte *data;
	size_t len;
	MIMEHeader mh;
	...
	SinkGetData(is->wp, &data, &len, &mh);
	if (is->if_vector.addDataProc && is->if_vector.image_format_closure) {
		is->if_vector.addDataProc(is->if_vector.image_format_closure, data, len, true);
	}
...
ImageClassDestroy(void *closure) {
	...
	for (i = 0; i < TCOUNT; i++) {
		if (ic->color_tables[i].generic_dither_table != NULL) {
			free(ic->color_tables[i].generic_dither_table);
		}
	}
	for (i = 0; i < TCOUNT; i++) {
		if (ic->gray_tables[i].generic_dither_table != NULL) {
			free(ic->gray_tables[i].generic_dither_table);
		}
	}
	MPDestroy(ic->mp);
...
int InitModule_Image(ChimeraResources cres) {
